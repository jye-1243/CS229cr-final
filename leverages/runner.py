...
from leverages.desc import LeverageDesc
...
@dataclass
class LeveragePruneRunner(Runner):
    replicate: int
    levels: int
    desc: LeverageDesc
    verbose: bool = True
    ...
    @staticmethod
    def description():
        return 'Run a leverage-based pruning experiment.'
    ...
    def add_args(parser: argparse.ArgumentParser) -> None:
        ...
        lottery_parser = parser.add_argument_group(
            'Leverage Ticket Hyperparameters', 'Hyperparameters that control the lottery ticket process.')
        LeveragePruneRunner._add_levels_argument(lottery_parser)
        LeverageDesc.add_args(parser, defaults)

    @staticmethod
    def create_from_args(args: argparse.Namespace) -> 'LeveragePruneRunner':
        print("CReATE FROM ARGS")
        return LeveragePruneRunner(args.replicate, args.levels, LeverageDesc.create_from_args(args),
    ...
    def run(self) -> None:
        print("RUN RUNRUNRUNRUNRUN")
        if self.verbose and get_platform().is_primary_process:
            print('='*82 + f'\nLeverage Ticket Experiment (Replicate {self.replicate})\n' + '-'*82)
    ...
    def _establish_initial_weights(self):
        print("ESTABLISH INITIAL WEIGHTS")
